...
export function createStructuredData(route: SeoRoute, siteConfig: SiteConfig) {
  ...
  if (schema.type === "Course") {
    return { "@context": "https://schema.org", "@type": schema.type, name: schema.name, description: schema.description, provider: { "@id": `${siteConfig.siteUrl}/#club` }, courseMode: schema.courseMode, about: schema.about, url: canonical };
  }
